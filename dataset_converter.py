...
# env_class_names = ['daytime_clear', 'daytime_foggy']
env_class_names = ['daytime_clear', 'night_rainy']
env_label_ids = {name: i for i, name in enumerate(env_class_names)}

def parse_xml(args):
    devkit_path, xml_path, img_path = args
    env_class = devkit_path.split('/')[-1]
    tree = ET.parse(xml_path)
    root = tree.getroot()
    # size = root.find('size')
    # w = int(size.find('width').text)
    # h = int(size.find('height').text)
    img = Image.open(osp.join(devkit_path, img_path))
    ...
    for obj in root.findall('object'):
        ...
        diffi = obj.find('difficult')
        difficult = int(diffi.text) if diffi is not None else 0
    ...
    annotation = {
        'filename': img_path,
        'env_label': env_label_ids[env_class],
        'width': w,
        'height': h,
        'ann': {
...
def cvt_annotations(devkit_path, years, split, out_file):
    ...
    for year in years:
        ...
        devkit_paths = [devkit_path] * len(xml_paths)
        part_annotations = track_progress(parse_xml,
                                          list(zip(devkit_paths, xml_paths, img_paths)))
...
def cvt_to_coco_json(annotations):
    ...
    for ann_dict in annotations:
        ...
        image_item['env_label'] = int(ann_dict['env_label'])
